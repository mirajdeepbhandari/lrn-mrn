...
const Blogs = () => {
     ...
     const debounced_sort_Term = useDebounce(sort_term, 400);
      ...
      const getBlogs = async ({ queryKey }) => { //coming from useQuery query params
        const [_key, { search_term , sort_term}] = queryKey;
        return await instance.get(`${URLS.GET_PUBLISHED_BLOGS}?title=${search_term}&sort=${sort_term}`);
      };

      // Queries // https://tanstack.com/query/latest/docs/framework/react/guides/query-functions
      const {data, isPending}= useQuery({ queryKey: ['published-blogs', {search_term: debounced_search_Term , sort_term: debounced_sort_Term}], queryFn: getBlogs });
